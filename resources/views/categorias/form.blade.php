...
@section('content')
    <form action="{{ !isset($categoria) ? '/categorias' : route('categorias.update', ['categoria' => $categoria]) }}"
        method="POST" class="rounded-md text-white mt-7 px-5 py-3 bg-gradient-to-r from-blue-500 to-purple-600">
        @if (!isset($categoria))
        @else
            @method('PUT')
        @endif
        ...
        <div class="mt-7 relative z-0 mb-6 w-full group">
            <input value="{{ !isset($categoria) ? '' : $categoria->nome }}" name="nome" id="floating_nome"
