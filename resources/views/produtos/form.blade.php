...
@section('content')
    <form action="{{ !isset($produto) ? '/produtos' : route('produtos.update', ['produto' => $produto]) }}" method="POST"
        enctype="multipart/form-data"
        class="rounded-md text-white mt-1 px-5 py-3 formulario bg-gradient-to-r from-blue-500 to-purple-600">
        @if (!isset($produto))
        @else
            @method('PUT')
        @endif
        ...
        <div class="mt-7 relative z-0 mb-6 w-full group">
            <input value="{{ !isset($produto) ? '' : $produto->preco }}" name="preco" id="floating_preco"
                class="block py-2.5 px-0 w-full text-sm text-white bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer">
        ...
        <div class="mt-7 relative z-0 mb-6 w-full group">
            <input value="{{ !isset($produto) ? '' : $produto->marca }}" name="marca" id="floating_marca"
                class="block py-2.5 px-0 w-full text-sm text-white bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer">
        ...
        <div class="mt-7 relative z-0 mb-6 w-full group">
            <input value="{{ !isset($produto) ? '' : $produto->descricao }}" name="descricao" id="floating_descricao"
                class="block py-2.5 px-0 w-full text-sm text-white bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer">
        ...
        <div class="mt-7 relative z-0 mb-6 w-full group">
            <select name="categoria" id="floating_categoria"
                class="block py-2.5 px-0 w-full text-sm text-white bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer">
                @foreach ($categorias as $categoria)
                    <option value="{{ $categoria->id }}">{{ $categoria->nome }}</option>
                @endforeach
            </select>
            <label for="floating_categoria"
        ...
        <div class="flex justify-center">
            <button
                class="align-items-center text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Enviar</button>
        </div>
